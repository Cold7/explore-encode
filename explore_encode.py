						...
						if len(response_json_dict["@graph"][i]["replicates"]) >= n_replicates:
							...
							while response2 == None:
								try:
									...
									print "error trying to get response from "+URL2+" retrying in 10 seconds"
									time.spleep(10)
					except:
						pass
					i += 1
			except:
				print "Some error trying to read a response... waiting 10 second until to retry do get data from "+URL
				time.spleep(10)
